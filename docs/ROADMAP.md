...
- **Visibility-change safety rail** `[needs brainstorm]` — today `markland_publish` accepts `public=true` in one tool call, so a casually-worded prompt can flip a doc public without a human gesture. Rough idea: two-step it (`markland_publish` ignores `public=true`, defaults private; `markland_doc_meta(public=…)` is the only way to flip) + flash + bold audit-log entry. Needs design pass: backwards-compat for existing tool callers, MCP deprecation path, what the UI flash actually looks like, whether to require interactive confirmation for cross-principal grants. Sourced from 2026-05-03 third-party concerns review.
...
- **2026-09-08** — **MCP SDK 1.27.0 → 2.2.0, protocol `2026-07-28` (PR #79) + Sentry disconnect-noise filter (PR #78)**. A Sentry "5xx spike" alert surfaced 102 paired `ClientDisconnect` / `Received exception from stream` events in one day against a ~1/day baseline. Root cause: Claude Code ≥2.1.263 opens a connection by probing `server/discover` at protocol `2026-07-28` (sessionless single-exchange POST) instead of the legacy `initialize` handshake; mcp 1.27.0 tops out at `2025-11-25`, answered `400 Bad Request: Missing session ID`, and the client hung up mid-body. The SDK's blanket `except Exception` around its POST handler then logged the disconnect as a server fault *and* re-sent it into the read stream, costing two ERROR events plus a 500 written to a closed socket. Upgrading fixes it at the source (2.2.0 emits zero ERROR records for the same disconnect); the `before_send` filter in `log_scrubbing.py` covers the window and guards an SDK downgrade. Migration surface was narrow — `FastMCP`→`MCPServer`, `ToolError` module move, and two settings becoming `streamable_http_app()` kwargs. Verified in prod with the SDK's own client: `auto` → `server/discover` @ `2026-07-28`, `legacy` → `initialize` @ `2025-11-25`, 23 tools on both. Pinned by `tests/test_mcp_protocol_negotiation.py`. Beads `markland-g1b`, `markland-xo2`; open follow-up `markland-93a` (empty `serverInfo.version`).
- **2026-07-06** — **MCP audit complete — Phase B shims removed (Plan 7, the final MCP-audit plan)**. Dropped `markland_set_visibility`, `markland_feature`, `markland_set_status`, `markland_clear_status` and the `markland_grant` `principal=` alias after the 30-day deprecation window (opened 2026-05-31, executed 2026-07-06). Surface is now **23 tools** (spec §8.0 target of 22 + `markland_admin_metrics`, which landed post-audit via PR #53). Closes the 10-plan MCP audit arc: envelopes, closed-set error model, pagination, per-tool idempotency contracts. Spec `docs/specs/2026-04-27-mcp-audit-design.md`; plan `docs/plans/2026-04-27-mcp-phase-b-deprecation-removal.md`; bead `markland-thh`.
