...
- **Phase 2 content cadence** `[slipped]` — `/blog` infrastructure shipped (PR #63) and post #1 ("What is agent-native publishing?") is live at `https://markland.dev/blog/agent-native-publishing`. Post #2 target was ~2026-05-17 (one post / 2 weeks per `docs/audits/2026-05-03-seo-strategy/SEO-STRATEGY.md` §4) — slipped by 2+ days as of 2026-05-19; nothing in `src/markland/web/content/blog/` beyond post #1. Working list of next anchor titles: "How to share Claude Code output without copy-pasting", "MCP, explained for developers", "Five things to publish to Markland from Claude Code", "Why markdown round-trips break in Notion", "Building a public CLAUDE.md library". Pick one to ship this week or push the next slot to ~2026-05-31.
...
- **Soak-window analytics check** `[overdue]` — beads `markland-fjd`, was due **2026-05-15** (4 days late as of 2026-05-19). Operator side now fully tooled: `docs/runbooks/metrics-review.md` + `scripts/admin/list_users.py` + `scripts/admin/umami_summary.py` landed 2026-05-18 — running the check is now `./scripts/admin/curl-admin "/admin/metrics?window_seconds=1209600" | jq` plus two helper script calls per the runbook. Partial data pulled 2026-05-05 (6 users, 12 docs, 3 private user docs); full 14-day check still pending. Sub-issues: `markland-fjd.1/.2/.3/.4`.
- **Self-service deletion** — Spec: `docs/specs/2026-05-04-self-service-deletion-design.md`. Plan: `docs/plans/2026-05-04-self-service-deletion.md`. Two-tier model: documents delete immediately and irreversibly (typed-confirmation), accounts get a 30-day soft-delete window (magic-link reverify, frozen-from-the-outside semantics, daily cron purges past-due accounts and anonymizes the `users` row to a tombstone — preserving the audit-log append-only invariant from PR #49). Three independently-mergeable phases: Phase 1 doc-delete UI (~5 tasks, no schema), Phase 2 account soft-delete + recovery (~10 tasks, schema migrations + service + auth gate + routes + templates), Phase 3 cron purge (~3 tasks, mirrors `presence_gc`). Email reuse permitted post-purge.
