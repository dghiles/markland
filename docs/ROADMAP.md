...
- **MCP `/mcp` 307-redirect fix** — beads `markland-dfj` (P2). Production logs show every Claude Code request hits a `POST /mcp → 307 → POST /mcp/` round trip (~5-8s of avoidable latency per session establishment). Fix: disable Starlette's mount-point trailing-slash redirect at `src/markland/web/app.py:875`, with an explicit `/mcp` route fallback if the cheap fix doesn't take. Plan: `docs/plans/2026-05-04-mcp-trailing-slash-redirect.md`.
