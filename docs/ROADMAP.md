...
- **Pre-launch clean up** — Plan: `docs/plans/2026-05-29-pre-launch-cleanup.md`. Six tracks sequenced as: Track A Phase 0 dogfood finish → Track B homepage CRO (replace "Pre-launch · we'll email" hero with magic-link sign-in CTA) → Track C blog post #2 ("How to share Claude Code output without copy-pasting") → Track D dashboard welcome / first-publish nudge → Track E trust gates in parallel (self-service deletion Phase 1, formal privacy policy, formal ToS) → Track F Show HN draft queued (do not post until tracks B-E shipped and funnel evidence confirms conversion). Roadmap close-out in Track G. Scope drove from 2026-05-29 soak-window: 91% bounce, 0 signups in 14d, 1 blog view in 14d — leak-at-hero, not trust-gates-blocking.
