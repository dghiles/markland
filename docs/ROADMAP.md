...
- **Pre-launch clean up** — bundle plan: `docs/plans/2026-05-29-pre-launch-cleanup.md`. 8 individually pickable tracks below, each with its own plan link. Sequencing: **A blocks B**; B + C + D + E run in parallel after A; F drafts during; G is close-out. Background: soak-window 2026-05-29 found 91% bounce on `/` and 0 signups in 14d — leak at the hero, not at the trust gates. Show HN (F) posts only after B–E ship and funnel evidence confirms conversion is fixed.
  ...
  - **[Pre-launch E1] Self-service deletion — Phase 1 only (doc-delete UI)** — Plan: `docs/plans/2026-05-04-self-service-deletion.md` Tasks 1.1–1.3 only. Surfaces the existing `service/docs.delete` API as a Delete button on `/dashboard` + viewer with typed-confirmation modal. Phase 2 (account soft-delete) + Phase 3 (cron purge) stay deferred — out of scope until first 25 organic signups land.
  - **[Pre-launch E2] Formal privacy policy** — Plan: `docs/plans/2026-05-04-formal-privacy-policy.md` (14 tasks). Promote `/privacy` from working-summary stub to standard 10-section document.
  - **[Pre-launch E3] Formal Terms of Service** — Plan: `docs/plans/2026-05-04-formal-terms-of-service.md` (17 tasks). Promote `/terms` from working-summary stub to standard 14-section document. Can ship in parallel with E2.
