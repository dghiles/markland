...
def test_mcp_no_slash_does_not_redirect(client):
    """POST /mcp must return 401 (auth required) directly, not 307."""
    r = client.post(
        "/mcp",
    ...
    # 401 (PrincipalMiddleware bearer-required) is the expected gate.
    assert r.status_code == 401, r.text


def test_mcp_slash_works_unchanged(client):
    """POST /mcp/ continues to work exactly as it did pre-fix."""
    r = client.post(
        "/mcp/",
        json={"jsonrpc": "2.0", "method": "initialize", "id": 1},
        follow_redirects=False,
...
def test_mcp_get_no_slash_does_not_redirect(client):
    """GET /mcp also must not 307 — some MCP probes use GET."""
...
Expected: `test_mcp_no_slash_does_not_redirect` and `test_mcp_get_no_slash_does_not_redirect` FAIL with status_code 307. `test_mcp_slash_works_unchanged` should PASS today.
